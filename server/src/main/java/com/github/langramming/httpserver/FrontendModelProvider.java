...
public class FrontendModelProvider {
    ...
    @Inject
    public FrontendModelProvider(UserService.UserProvider userProvider, SpotifyUserService spotifyUserService) {
    ...
    private FrontendModel.FrontendUserModel toUserModel(User user) {
        return FrontendModel.FrontendUserModel.builder()
                .id(user.getId())
                .name(user.getName())
