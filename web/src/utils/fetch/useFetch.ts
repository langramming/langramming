...
export const useFetch = <T>(uri: string): State<T> => {
    const castReducer = reducer as (prevState: State<T>, action: ActionType<T>) => State<T>;
    const [{ loading, data, error }, dispatch] = React.useReducer(castReducer, { loading: false, data: undefined, error: undefined });
